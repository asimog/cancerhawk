...
export function SiteBackground() {
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      const radius = Math.min(width, height) * (0.11 + bass * 0.08);
      const bloom = drawingContext.createRadialGradient(cx, cy, radius * 0.2, cx, cy, Math.max(width, height) * 0.42);
      bloom.addColorStop(0, `rgba(73,197,182,${0.16 + energy * 0.16})`);
      bloom.addColorStop(0.42, `rgba(39,121,167,${0.08 + audio.mid * 0.14})`);
      ...
      for (let i = 0; i < 170; i += 1) {
        ...
        const lane = 1.5 + (i % 19) * 0.055 + energy * 0.22;
